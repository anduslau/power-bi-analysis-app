...
class AnthropicClient(BaseLLMClient):
    ...
    def max_context_tokens(self) -> int:
        return self._context_windows.get(self._model, 100000)

    def estimate_tokens(self, text: str) -> int:
        """
        Estimate token count using Anthropic's tokenizer.
        """
        # Anthropic provides a tokenizer
        try:
    ...
    def generate_brd(self, metadata_text: str, file_name: str) -> str:
        """
        Generate Business Requirements Document using Claude.
        """
    ...
    def generate_sml_yaml(self, metadata_text: str, file_name: str) -> str:
        """
        Generate Semantic YAML using Claude.
        """
        ...
        try:
            response = self._client.messages.create(
                ...
                system=self._get_sml_system_prompt(),
                messages=[
                    {"role": "user", "content": prompt}
                ]
            )
    ...
    def generate_sql_schema(self, metadata_text: str, file_name: str) -> str:
        """
        Generate SQL schema (DDL) using Claude.
        """
        ...
        try:
            response = self._client.messages.create(
                ...
                system=self._get_sql_schema_system_prompt(),
                messages=[
                    {"role": "user", "content": prompt}
                ]
            )
    ...
    def generate_data_dictionary(self, metadata_text: str, file_name: str) -> str:
        """
        Generate data dictionary using Claude.
        """
        ...
        try:
            response = self._client.messages.create(
                ...
                max_tokens=4096,
                temperature=0.2,
                system=self._get_data_dictionary_system_prompt(),
                messages=[
                    {"role": "user", "content": prompt}
                ]
            )
